...
// Fetch customization data and apply it to the page
const fetchData = async () => {
  try {
    const response = await fetch("customize.json");
    if (!response.ok) throw new Error("Failed to fetch customization data");
    ...
    dataArr.forEach((customData, index) => {
      ...
      // Start animation once all elements are updated
      if (index === dataArr.length - 1) animationTimeline();
    });
  } catch (error) {
    console.error(error);
  }
...
const animationTimeline = () => {
  ...
  textBoxChars.innerHTML = textBoxChars.textContent.split("").map(char => `<span>${char}</span>`).join("");
  ...
  tl.to(".container", 0.1, { visibility: "visible" })
    .from(".one, .two", 0.7, { opacity: 0, y: 10 }, 0.2)
    ...
    .from(".idea-4", 0.7, ideaTextTrans)
    .to(".idea-4", 0.7, ideaTextTransLeave, "+=1.5")
    .from(".idea-5", 0.7, { rotationX: 15, rotationZ: -10, skewY: "-5deg", y: 50, opacity: 0 }, "+=0.5")
